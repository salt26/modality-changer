...
# https://github.com/hyungkwonko/M1522.000500-infovis/blob/master/Preprocessing/preprocess_midi.py
...
unit_duration = 16  # 16 (16nd note) or 32 (32nd note)
...
#       line 192:  with  open(midi_file, 'rb') as f:
#       line 220:    tf.io.gfile.Copy(temp_file.name, output_file, overwrite=True)
...
# Get token sequence and write .mid file. Return a path of the file
...
def read_dir(in_dir, verbose=False, one_hot=False, valence_label=True, arousal_label=True, predict_emotion=False):
    ...
    if valence_label and arousal_label:
        valence_preprocess_result, arousal_preprocess_result = get_preprocessed_valence_arousal_dict("./data/vgmidi_annotations")
    ...
    for i, in_path in enumerate(mid_files):
        ...
        if valence_label and not (fname in valence_preprocess_result):
            print("WARNING : skipped >>>>> "+ fname+" >>>>>  (has no valence annotations)")
            continue
        elif arousal_label and not (fname in arousal_preprocess_result):
            print("WARNING : skipped >>>>> "+ fname+" >>>>>  (has no arousal annotations)")
            continue
        ...
        try:
            ...
            if valence_label:
                if fname == "Legend_Of_Zelda_The_Ocarina_Of_Time_PreludeofLight":
                    v_valence_list.extend(valence_preprocess_result[fname])
                else:
                    v_valence_list.extend([v for (j, v) in enumerate(valence_preprocess_result[fname]) if valid_sequences[j]])
            if arousal_label:
                if fname == "Legend_Of_Zelda_The_Ocarina_Of_Time_PreludeofLight":
                    v_arousal_list.extend(arousal_preprocess_result[fname])
                else:
                    v_arousal_list.extend([v for (j, v) in enumerate(arousal_preprocess_result[fname]) if valid_sequences[j]])
            ...
            if valence_label:
                assert(len(v_valence_list) == len(v_midi_list))
            if arousal_label:
                assert(len(v_arousal_list) == len(v_midi_list))
    ...
    f = open(OUT_DIR+"extracted/metadata", 'w')
...
def sequences_to_midi_file(events, sequences_length, tpb, original_filename, create_files=False):
    ...
    for event in events:
        # print(event)
        if current_seq_index != event['Seq_index']:
            current_seq_index = event['Seq_index']
            print(str(event['Seq_index']) + "-------------------------------------------------------------")
        tp = event['Type']
        if tp == 'note_on':
            msg = Message(tp, channel=event['Channel'], note=event['Note_position'],
                                 velocity=event['Note_velocity'], time=event['Delta_tick'])
            track.append(msg)
            print(msg)
            single_note_on.append((event['Note_position'], event['Channel']))
        if tp == 'note_off':
            ...
            msg = Message(tp, channel=event['Channel'], control=event['Control'],
                                 value=event['Value'], time=event['Delta_tick'])
            track.append(msg)
            print(msg)
        elif tp == 'set_tempo':
            msg = MetaMessage(tp, tempo=event['Tempo'], time=event['Delta_tick'])
            track.append(msg)
            print(msg)
        elif tp == 'end_of_track':
            msg = MetaMessage(tp, time=event['Delta_tick'])
            track.append(msg)
            print(msg)
        elif event['Is_meta']:
            msg = MetaMessage(tp, time=event['Delta_tick'])
    ...
    for msg in merge_tracks(midi.tracks):
        ...
        if current_seq_index != unit_index(current_tick, tpb) // unit_duration:
            current_seq_index = unit_index(current_tick, tpb) // unit_duration
            print(str(unit_index(current_tick, tpb) // unit_duration) + "-------------------------------------------------------------")

        tp = msg.type
        if tp == 'note_on':
            msg2 = Message(tp, channel=msg.channel, note=msg.note,
                                 velocity=msg.velocity, time=msg.time)
            track.append(msg2)
            print(msg2)
            single_note_on.append((msg.note, msg.velocity, msg.channel))
        if tp == 'note_off':
            if (msg.note, msg.velocity, msg.channel) in single_note_on:
                msg2 = Message(tp, channel=msg.channel, note=msg.note,
                                     velocity=msg.velocity, time=msg.time)
                track.append(msg2)
                print(msg2)
                single_note_on.remove((msg.note, msg.velocity, msg.channel))
        elif tp == 'control_change':
            msg2 = Message(tp, channel=msg.channel, control=msg.control,
                                 value=msg.value, time=msg.time)
            track.append(msg2)
            print(msg2)
        elif tp == 'set_tempo':
            msg2 = MetaMessage(tp, tempo=msg.tempo, time=msg.time)
            track.append(msg2)
            print(msg2)
        elif tp == 'end_of_track':
            msg2 = MetaMessage(tp, time=msg.time)
            track.append(msg2)
            print(msg2)
        elif msg.is_meta:
            msg2 = MetaMessage(tp, time=event['Delta_tick'])
            track.append(msg2)
            print(msg2)
    """
...
def extract_rhythm_density(sequences, onset_sequences, sequences_length, verbose=False, one_hot=False, valid_sequences=None):
    ...
    note_velocity_vector = np.zeros((sequences_length, unit_duration, 9))
    tempo_vector = np.zeros((sequences_length, 1, 6))
    key_vector = np.zeros((sequences_length, 1, 12))
...
def extract_melodic_contour(sequences, onset_sequences, sequences_length, verbose=False, one_hot=False, valid_sequences=None):
    ...
    #melodic_contour_vector_oh = np.zeros((sequences_length, unit_duration, 3))
...
# TODO velocity vector 만들 때 각 시간 단위의 모든 음표의 velocity 평균을 구해야 한다. (0 제외)
...
def aggregate_note_density(note_density_vector):
    # input shape: (sequence_length, unit_duration)
    # output shape: (sequence_length, )
    return np.array([sum(k) / len(k) for k in note_density_vector])


def aggregate_tempo(tempo_vector):
    # input shape: (sequence_length, unit_duration)
    # output shape: (sequence_length, )
    return np.array([np.bincount(k).argmax() for k in tempo_vector])


def aggregate_maj_min_1(chord_vector):
    # input shape: (sequence_length, unit_duration)
    # output shape: (sequence_length, )
    # using chord feature
    # -unit_duration ~ unit_duration: (# of maj chords) - (# of min chords) - (# of dim chords)
    a_chord = np.zeros(chord_vector.shape[0])
    for i in range(chord_vector.shape[0]):
        for j in range(chord_vector.shape[1]):
            if 1 <= chord_vector[i, j] <= 12:
                a_chord[i] += 1  # major
            elif 13 <= chord_vector[i, j] <= 24:
                a_chord[i] -= 1  # minor
            elif 37 <= chord_vector[i, j] <= 48:
                a_chord[i] -= 1  # diminished
    return np.divide(a_chord, chord_vector.shape[1])


def aggregate_maj_min_2(key_vector):
    # input shape: (sequence_length, )
    # output shape: (sequence_length, )
    # 0: no key, 1: major(using key feature), 2: minor(using key feature)
    a_key = np.zeros(key_vector.shape[0])
    for i in range(key_vector.shape[0]):
        if key_vector[i] == 0:
            a_key[i] = 0
        else:
            a_key[i] = ((key_vector[i] - 1) // 12) + 1
    return a_key


def extract_tonic(key_vector):
    # input shape: (sequence_length, )
    # output shape: (sequence_length, )
    # 0: no key, 1: C, 2: C#, ..., 12: B
    a_tonic = np.zeros(key_vector.shape[0])
    for i in range(key_vector.shape[0]):
        if key_vector[i] == 0:
            a_tonic[i] = 0
        else:
            a_tonic[i] = ((key_vector[i] - 1) % 12) + 1
    return a_tonic


def extract_chord_quality(chord_vector):
    # input shape: (sequence_length, unit_duration)
    # output shape: (sequence_length, unit_duration)
    # 0: no chord, 1: maj, 2: min, 3: aug, 4: dim, 5: sus4, 6: dom7, 7: min7
    a_chord_quality = np.zeros((chord_vector.shape[0], chord_vector.shape[1]), dtype=np.int64)
    for i in range(chord_vector.shape[0]):
        for j in range(chord_vector.shape[1]):
            if chord_vector[i, j] == 0:
                a_chord_quality[i, j] = 0
            else:
                a_chord_quality[i, j] = ((chord_vector[i, j] - 1) // 12) + 1
    return a_chord_quality
